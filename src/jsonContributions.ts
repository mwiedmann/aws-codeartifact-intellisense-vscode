...
export function addJSONProviders(): Disposable {
  const contributions = [new PackageJSONContribution()]
  const subscriptions: Disposable[] = []
  contributions.forEach((contribution) => {
    const selector = contribution.getDocumentSelector()
    subscriptions.push(
      languages.registerCompletionItemProvider(selector, new JSONCompletionItemProvider(contribution), '"', ':'),
    )
  ...
  public async provideCompletionItems(
      ...
      add: (suggestion: CompletionItem) => {
        ...
        if (!proposed[key]) {
          ...
          suggestion.range = {
            ...
            inserting: new Range(overwriteRange.start, overwriteRange.start),
          }
    ...
    if (location.isAtPropertyKey) {
      ...
      const addValue =
        ...
        !this.hasColonAfter(scanner, location.previousNode.offset + location.previousNode.length)
